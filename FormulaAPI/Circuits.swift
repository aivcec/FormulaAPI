...
extension FormulaAPI {
    ...
    public static func driverCircuitsObs(driverId: String, type: FormulaType, year: Int? = nil) -> Observable<[Circuit]> {
        let circuitsPath = buildAPIprefix(type: type, year: year) + "drivers/\(driverId)/circuits.json"
